...
if [ -z ${OLD_GIT_ORG} ]; then echo "Please set OLD_GIT_ORG when running script, optional GIT_BASEURL and GIT_REPO to formed the git url GIT_BASEURL/GIT_ORG/*"; exit 1; fi
...
find ${SCRIPTDIR}/../0-bootstrap -name '*.yaml' -print0 |
  while IFS= read -r -d '' File; do
    if grep -q "kind: Application\|kind: AppProject" "$File"; then
      #echo "$File"
      sed -i'.bak' -e "s#${GIT_BASEURL}/${OLD_GIT_ORG}/${GIT_GITOPS}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS}#" $File
      # sed -i'.bak' -e "s#${GIT_GITOPS_BRANCH}#${GIT_GITOPS_BRANCH}#" $File
      sed -i'.bak' -e "s#${GIT_BASEURL}/${OLD_GIT_ORG}/${GIT_GITOPS_INFRA}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_INFRA}#" $File
      # sed -i'.bak' -e "s#${GIT_GITOPS_INFRA_BRANCH}#${GIT_GITOPS_INFRA_BRANCH}#" $File
      sed -i'.bak' -e "s#${GIT_BASEURL}/${OLD_GIT_ORG}/${GIT_GITOPS_SERVICES}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_SERVICES}#" $File
      # sed -i'.bak' -e "s#${GIT_GITOPS_SERVICES_BRANCH}#${GIT_GITOPS_SERVICES_BRANCH}#" $File
      sed -i'.bak' -e "s#${GIT_BASEURL}/${OLD_GIT_ORG}/${GIT_GITOPS_APPLICATIONS}#${GIT_BASEURL}/${GIT_ORG}/${GIT_GITOPS_APPLICATIONS}#" $File
